...
export default function AddForoForm(props) {
  const { toastRef, setIsLoading, navigation } = props;
  const [foroName, setForoName] = useState("");
  const [foroMateria, setForoMateria] = useState("");
  const [foroDescription, setForoDescription] = useState("");
  const [imageSelected, setImageSelected] = useState([]);

  const AddForo = () => {
    if (!foroName || !foroMateria || !foroDescription) {
  ...
  return (
    <ScrollView style={styles.scrollView}>
      <ImageForo imagenForo={imageSelected[0]} />
      <FormAdd
        setForoName={setForoName}
...
function ImageForo(props) {
  const { imagenForo } = props;
...
function FormAdd(props) {
  const { setForoName, setForoMateria, setForoDescription } = props;
  return (
    <View style={styles.viewForm}>
      <Input
        placeholder="Nombre del foro"
        containerStyle={styles.input}
        onChange={(e) => setForoName(e.nativeEvent.text)}
      />
      <Input
        placeholder="Materia"
        containerStyle={styles.input}
